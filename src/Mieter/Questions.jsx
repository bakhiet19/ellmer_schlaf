...
const Questions = [
  ...
  { key: "notes", question: "Haben Sie besondere Wünsche?", type: "textarea", placeholder: "z. B. Erdgeschoss, Parkplatz, Frühstück..." },
];

const schema = z.object({
  name: z.string().min(2, { message: "Bitte geben Sie einen gültigen Namen ein." }),
  firmaName: z.string().min(2, { message: "Bitte geben Sie eine gültige Firmenbezeichnung ein." }),
  ...
  peopleCount: z.preprocess(
    (val) => Number(val),
    z.number().min(1, { message: "Bitte geben Sie die Anzahl der Personen ein." })
  ),
  checkIn: z.string().min(1, { message: "Bitte wählen Sie ein Check-in-Datum." }),
  checkOut: z.string().min(1, { message: "Bitte wählen Sie ein Check-out-Datum." }),
  budget: z.preprocess(
    (val) => Number(val),
    z.number().min(1, { message: "Bitte geben Sie ein gültiges Budget ein." })
  ),
...
export default function QuestionStep({ className }) {
  ...
  return (
   <div
  className={`w-full px-6 py-12 rounded-xl shadow-lg bg-gradient-to-br from-white via-gray-50 to-gray-100 border border-gray-200 ${className}`}
>
  {/* Progress Bar */}
  <div className="bg-gray-200 rounded-full h-2 mb-6">
    <div
      className="logoBG h-2 rounded-full transition-all duration-300"
    ...
    <form className="space-y-6">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {errors[currentQuestion.key] && (
            <p className="text-sm logoText mt-1">
      ...
      {step === totalSteps - 1 && (
        <div className="space-y-4">
          <label className="flex items-start gap-3 text-gray-700">
            ...
            <span>
              Ja, ich habe die{" "}
              <a href="/agb" className="text-rose-600 underline">
                AGB
              </a>{" "}
      ...
      <div className="flex justify-between pt-4">
        <button
          ...
          onClick={onBack}
          disabled={step === 0}
          className="flex items-center gap-2 sm:px-6 sm:py-3 px-1 py-1 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition disabled:opacity-50"
        >
    ...
    <form onSubmit={handleSubmit(onFinalSubmit)} className="space-y-6">
      ...
      <div className="space-y-4 pt-4">
        ...
        <label className="flex items-start gap-3 text-gray-700">
          <input
            type="checkbox"
            {...register("datenschutz")}
            defaultChecked={getValues("datenschutz")}
            className="mt-1 accent-rose-500"
          />
          <span>
            Ich stimme der Datenverarbeitung zu und wünsche Kontaktaufnahme.
      ...
      <div className="flex justify-between pt-6">
        ...
        <button
          ...
          className={`sm:px-6 sm:py-3 px-1 py-1 rounded-lg font-medium transition ${
            mutation.isLoading
